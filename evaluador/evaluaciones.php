...
$evaluacion_query = mysqli_query($link, "SELECT evaluacion.*, empresa.nombre AS empresa, ley.nombre AS ley FROM evaluacion, empresa, ley, asignar_evaluador WHERE evaluacion.id_empresa = empresa.id AND evaluacion.id_ley = ley.id AND asignar_evaluador.id_empresa = empresa.id AND asignar_evaluador.id_usuario = $s_id");
